...
asl_test_path = cfg['ASLHG_TEST_DATA']
asl_train_path = cfg['ASLHG_TRAIN_DATA']

def normalize_landmarks(coord_list):
    """
    Takes 63 floats [x0,y0,z0,x1,y1,z1,...] and normalizes them relative to the wrist landmark
    ...
    # Double subtraction empirically improves accuracy.
    # Preserves absolute depth offset that full centering removes.
    zs = [z - wrist_z for z in zs]
...
def init_resnet18():
    """
    Set up dataloaders for resnet18 model
    ...
    train_ds = ImageFolder(asl_train_path, transforms)

    test_ds = ImageFolder(asl_test_path, v2.Compose([
        v2.Resize((224, 224), antialias=True),
        v2.ToImage(),
        v2.ToDtype(torch.float32, scale=True),
        v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    tst_loader = DataLoader(test_ds, batch_size=cfg['batch_size'], shuffle=True)
...
def init_lmc():
    """
    Initialize mediapipe parameters and executes landmark processing for the LandmarkClassifier
    """
    # mediapipe stuff
    ...
    with HandLandmarker.create_from_options(options) as landmarker:
        get_landmarks(landmarker)


def get_landmarks(landmarker: HandLandmarker):
    """
    Processes and saves a list of classes with their respective hand landmarks per image
    for use by the LandmarkClassifier.
    """
    ...
    for class_folder in sorted(os.listdir(asl_train_path)):
        cls_img_tr = os.path.join(asl_train_path, class_folder)
        cls_img_ts = os.path.join(asl_test_path, class_folder)
        ...
        log.info(f"Loaded landmarks for class: {class_folder}")
...
if __name__ == "__main__":
    lm_path_tr = cfg['ASLHG_LM_TRAIN']
    lm_path_tst = cfg['ASLHG_LM_TEST']

    # Process landmarks only if no cached version exists
    if not os.path.exists(lm_path_tr) or not os.path.exists(lm_path_tst):
        log.info(msg="Processing landmarks...")
        init_lmc()
        log.info(msg="Landmarks loaded!")
